...
- **RED:** Write a failing test for `get_token_from_environment_variable()`.
...
- **GREEN:** Implement the function and export it.
  ...
  - Remove `get_password_from_environment_variable()` from any production
...
- **RED:** Remove the two old tests from `test_environment.py`:
  `test_get_user_from_environment_variable` and
  `test_get_password_from_environment_variable`.
  - *Fails because:* The count of passing tests drops or the old functions
    are no longer covered.
- **GREEN:** Remove the old production functions:
  `get_user_from_environment_variable()` and
